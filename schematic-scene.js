...
(() => {
  ...
  const { evaluate } = window.ArcverexSession;
  ...
  class Schematic {
    ...
    draw(context, scene, session, palette, mobile) {
      const { time, reduced, live, example, latched, travel } = session;
      ...
      const fade = reduced ? 1 : 1 - smooth((time - 27) / 0.9);
      ...
      for (const path of this.paths) {
        ...
        const activated = live && (path.net === 'q' || travel > path.stage / 8);
      ...
      if (time > 14.5 && time < 18 && !reduced) {
        ...
        const y = check * 434 - 24;
        const envelope = smooth(check / 0.07) * smooth((1 - check) / 0.07);
        const scanColor = dark ? '#c4ffe0' : '#1d7651';
        c.save();

        // A broad inspection band reads clearly against both theme backgrounds.
        const band = c.createLinearGradient(0, y - 38, 0, y + 7);
        band.addColorStop(0, dark ? '#85eeb500' : '#25805a00');
        band.addColorStop(0.82, dark ? '#85eeb52e' : '#25805a29');
        band.addColorStop(1, dark ? '#85eeb500' : '#25805a00');
        c.globalAlpha = envelope;
        c.fillStyle = band;
        c.fillRect(-6, y - 38, 436, 45);

        // Illuminate the actual wires and gates under the scanner, with a trail.
        c.save();
        c.beginPath(); c.rect(-6, y - 25, 436, 29); c.clip();
        c.strokeStyle = scanColor; c.lineWidth = 1.65;
        c.shadowColor = scanColor; c.shadowBlur = dark ? 7 : 2;
        c.globalAlpha = envelope * 0.95;
        for (const path of this.paths) c.stroke(path.path);
        c.restore();

        c.globalAlpha = envelope * 0.95;
        c.strokeStyle = scanColor; c.lineWidth = 1.5;
        c.shadowColor = scanColor; c.shadowBlur = dark ? 9 : 2;
        c.beginPath(); c.moveTo(-6, y); c.lineTo(430, y); c.stroke();
        c.shadowBlur = 0;
        // Small bright end brackets distinguish the scanner from circuit nets.
        c.lineWidth = 2;
        for (const x of [-6, 430]) {
